...
const CreateCampaign = () => {
    ...
    const handleCreate = async () => {
        ...
        try {
            ...
            const response = await axios.post(`http://127.0.0.1:8080/api/campaigns`, formData, {
                headers: {
    ...
    const handleCancel = () => {
        ...
        console.log('Cancelled campaign creation');
    };
            ...
            <Box sx={{ flexGrow: 1, padding: 3, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                <Paper elevation={0} sx={{ padding: 4, width: '100%', maxWidth: '800px', border: '1px solid #e0e0e0', borderRadius: '4px' }}>
                    ...
                    <Typography variant="subtitle1" color='grey[200]' gutterBottom>
                       Enter your prompt to create a campaign. <a href="/doc" style={{ textDecoration: 'none', color: 'blue' }}>Visit docs ↗</a>
                    </Typography>
                    <div style={{ height: 30 }} />
                    <TextField
                        fullWidth
